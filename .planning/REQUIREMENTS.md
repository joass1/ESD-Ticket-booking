...
- [x] **EVNT-01**: User can browse and filter events by status, category, and date
- [x] **EVNT-02**: Admin can create, update, and manage events via OutSystems dashboard calling Event Service REST API
- [ ] **EVNT-03**: Admin can cancel an event, triggering `event.cancelled.{event_id}` on `event_lifecycle` topic exchange

### Seat Management

- [x] **SEAT-01**: User can view all seats for an event with current availability status
- [x] **SEAT-02**: User can view available seat count per section for an event
- [x] **SEAT-03**: Seat reservation uses Redis distributed lock (SET NX EX, 10 min TTL) followed by MySQL pessimistic lock (`FOR UPDATE`)
- [x] **SEAT-04**: If requested seat is taken, system auto-assigns next best available seat in same section using `SKIP LOCKED`
- [x] **SEAT-05**: Seat release deletes Redis lock and resets MySQL status to available (compensating transaction)
- [x] **SEAT-06**: Seat confirmation updates status to booked and removes Redis lock
...
- [x] **BOOK-01**: User can initiate a booking, triggering the orchestration saga (reserve seat -> process payment -> confirm booking)
- [x] **BOOK-02**: Booking Orchestrator performs compensating transactions on failure (release seat on payment failure, refund on booking failure)
- [x] **BOOK-03**: Booking Orchestrator detects payment expiry via APScheduler (30s interval) and releases held seats after 10 minutes
- [x] **BOOK-04**: Booking Orchestrator publishes `booking.confirmed` to `booking_topic` exchange after successful booking
- [x] **BOOK-05**: Booking Orchestrator publishes `booking.timeout` to `booking_topic` exchange on payment expiry
- [x] **BOOK-06**: User can view booking status and history
- [ ] **BOOK-07**: Booking status updates to `pending_refund` on event cancellation and `refunded` after refund completion

### Payment

- [x] **PAY-01**: Payment Service charges user via Stripe PaymentIntent (test mode, SGD currency)
...
| SEAT-01 | Phase 2 | Complete |
| SEAT-02 | Phase 2 | Complete |
| SEAT-03 | Phase 2 | Complete |
| SEAT-04 | Phase 2 | Complete |
| SEAT-05 | Phase 2 | Complete |
| SEAT-06 | Phase 2 | Complete |
